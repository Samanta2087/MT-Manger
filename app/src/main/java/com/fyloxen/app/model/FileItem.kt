...
data class FileItem(
    ...
    val childCount: Int = 0   // passed from FileUtils — never computed here
) {
    val name: String         = file.name
    val isDirectory: Boolean = file.isDirectory
    val size: Long           = if (file.isDirectory) -1L else file.length()
    ...
    fun formattedSize(): String {
        ...
        return when {
            size < 1024L              -> "$size B"
            size < 1024L * 1024       -> "${SIZE_FMT.format(size / 1024.0)} KB"
            size < 1024L * 1024 * 1024-> "${SIZE_FMT.format(size / (1024.0 * 1024))} MB"
            else                      -> "${SIZE_FMT.format(size / (1024.0 * 1024 * 1024))} GB"
        }
    }

    fun formattedDate(): String = DATE_FMT.format(Date(lastModified))
    ...
    companion object {
        // Shared formatters — created once, reused for every item (no per-bind allocation)
        private val DATE_FMT = SimpleDateFormat("MMM dd, yyyy HH:mm", Locale.getDefault())
        private val SIZE_FMT = DecimalFormat("#.#")
