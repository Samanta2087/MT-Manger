...
object FileUtils {
    ...
    fun listFiles(directory: File, showHidden: Boolean, sortOrder: SortOrder): List<FileItem> {
        ...
        // Single pass — NO extra listFiles() calls for subdirectories.
        // childCount is intentionally left at 0; the adapter handles display.
    ...
    private val SIZE_FMT = java.text.DecimalFormat("#.#")

    fun formatSize(bytes: Long): String {
        return when {
            bytes < 0            -> ""
            bytes < 1024L        -> "$bytes B"
            bytes < 1024L * 1024 -> "${SIZE_FMT.format(bytes / 1024.0)} KB"
            bytes < 1024L * 1024 * 1024 -> "${SIZE_FMT.format(bytes / (1024.0 * 1024))} MB"
            else -> "${SIZE_FMT.format(bytes / (1024.0 * 1024 * 1024))} GB"
        }
    }
    ...
    fun getQuickFolderSize(directory: File): Long {
        var size = 0L
